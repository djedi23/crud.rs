...
/*
This peice of code is refactored from https://github.com/alexhallam/tv
commit: 973d88f7ed05a394b5309c5e8b6f3e3a52a39b17
*/
...
pub fn is_na(text: &str) -> bool {
  lazy_static! {
    static ref R: Regex =
      Regex::new(r"^$|^(?:N(?:(?:(?:one|AN|a[Nn]|/A)|[Aa])|ull)|n(?:ull|an?|/a?)|(?:missing))$")
        .unwrap();
  }
  R.is_match(text)
}
...
pub fn is_na_string_padded(text: &str) -> bool {
  lazy_static! {
    static ref R: Regex =
      Regex::new(r"^$|(^|\s)(?:N(?:(?:(?:AN|a[Nn]|/A)|[Aa])|ull)|n(?:ull|an?|/a?)|(?:missing))\s*$")
        .unwrap();
  }
  R.is_match(text)
}
